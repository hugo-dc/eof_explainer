...
fn main() {
    test_legacy_contracts();
    test_no_eof_magic();
    test_eof1_container();
    println!("All tests were executed correctly");
}
...
fn is_invalid_with_error(code: Vec<u8>, expected_error: &str) {
    ...
    let mut err_message: &str = "no-error";
    ...
    let message = "Expected error `".to_owned() + expected_error + "`, found: `" + err_message + "`.";
    assert_eq!(error, true, "{}", message);
    assert_eq!(expected_error == err_message, true, "{}", message);
}

fn test_legacy_contracts() {
    let validator = Validator::new();

// ANCHOR: legacy_contracts
...
// ANCHOR_END: no_eof_magic
}

fn test_eof1_container() {
// ANCHOR: eof1_container
